...
class MainWindow(QMainWindow):
    ...
    def setup_roi_tab(self):
        ...
        if hasattr(self, 'detector') and self.detector is not None:
            self.roi_editor.video_source.detector = self.detector
    ...
    def process_frame(self):
        ...
        if self.detection_active and self.detector is not None and self.video_source.connection_ok:
            ...
            if detections:
                frame = self.detector.draw_detections(frame, detections, detections_in_roi)
                    ...
                    if roi.should_alert(current_time):
                        ...
                        thread = threading.Thread(
                            ...
                            args=(roi_idx, roi.name, roi.class_counts.copy(),
                                  self.video_source.camera_id, frame.copy())
                        )
                        thread.daemon = True
                        thread.start()
